...
var AiTextGeneration = class {
	...
	postProcessing(response) {
		if (this.modelSettings.postProcessingFunc) {
			this.postProcessedOutputs = {
				response: this.modelSettings.postProcessingFunc(response, this.preProcessedInputs),
			};
		} else {
	...
	postProcessingStream(response, inclen) {
		if (this.modelSettings.postProcessingFuncStream) {
			return {
				response: this.modelSettings.postProcessingFuncStream(response, this.preProcessedInputs, inclen),
			};
		} else {
...
var AiTextEmbeddings = class {
	...
	generateTensors(preProcessedInputs) {
		if (this.modelSettings.generateTensorsFunc) {
			...
			return [
				new Tensor('str' /* String */, Array.isArray(preProcessedInputs.text) ? preProcessedInputs.text : [preProcessedInputs.text], {
					shape: [Array.isArray(preProcessedInputs.text) ? preProcessedInputs.text.length : [preProcessedInputs.text].length],
					name: 'input_text',
				}),
			];
		}
...
var AiImageClassification = class {
	...
	postProcessing(response) {
		...
		for (const s in scores) labels.push({ label: resnetLabels[s], score: scores[s] });
		labels.sort((a, b) => {
			...
			if (preProcessedInputs.image) {
				tokens = [
					...
					...[
						...
						new Tensor('float32' /* Float32 */, [preProcessedInputs.strength || this.schema.input.properties.strength.default], {
							shape: [1],
							name: 'strength',
						}),
						new Tensor('float32' /* Float32 */, [preProcessedInputs.guidance || this.schema.input.properties.guidance.default], {
							shape: [1],
							name: 'guidance',
						}),
						new Tensor('uint8' /* Uint8 */, preProcessedInputs.image, {
...
var modelMappings = {
	...
	'text-embeddings': {
		models: ['@cf/baai/bge-small-en-v1.5', '@cf/baai/bge-base-en-v1.5', '@cf/baai/bge-large-en-v1.5', '@hf/baai/bge-base-en-v1.5'],
...
var Ai = class {
	...
	async run(model, inputs) {
		...
		const res = await this.binding.fetch('http://workers-binding.ai/run?version=2', fetchOptions);
